...
if( 'function' === typeof importScripts) {
    importScripts('loadGap.min.js', 'localSearch.js', 'knapsackDP.min.js');
    ...
    this.onmessage = function (input) {
        ...
        cap = gv.cap; // vattore delle capacità
...
{  var i,j;
   ...
   {  for(j=0;j<n;j++)
      ...
      zlb -= KDynRecur(cap[i], Q, val, solK); // segno inverso perchè era di di min
      for(j=0;j<n;j++)
         if(solK[j] > 0)
   ...
   for(j=0;j<n;j++)
      zlb += lambda[j];       // somma penalità in funzione lagrangiana
